...
def main():
    ...
    for N in [100, 1000]:
        ...
        trapezoidal_error = np.abs(trapezoidal_result - exact_integral) / exact_integral
        simpson_error = np.abs(simpson_result - exact_integral) / exact_integral

        print(f"N = {N}")
        print(f"梯形法则结果: {trapezoidal_result:.8f}, 相对误差: {trapezoidal_error:.2e}")
        print(f"Simpson 法则结果: {simpson_result:.8f}, 相对误差: {simpson_error:.2e}")
        print("-" * 40)
